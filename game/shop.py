...
class ShopView(GameView):
    ...
    async def _on_buy_root(self, interaction: discord.Interaction):
        ok, tier, name = self.game.buy_root_reroll(self.user_id, self.display_name)
        if ok:
    ...
    async def _on_buy_root_batch(self, interaction: discord.Interaction):
        rolls, hit_target = self.game.buy_root_reroll_batch(self.user_id, self.display_name, self.root_target_tier)
        if not rolls:
            self.last_result = "Not enough spirit stones."
        else:
    ...
    async def _on_buy_physique(self, interaction: discord.Interaction):
        ok, tier, name = self.game.buy_physique_reroll(self.user_id, self.display_name)
        if ok:
    ...
    async def _on_buy_physique_batch(self, interaction: discord.Interaction):
        rolls, hit_target = self.game.buy_physique_reroll_batch(self.user_id, self.display_name, self.physique_target_tier)
        if not rolls:
            self.last_result = "Not enough spirit stones."
        else:
